...
def insecure_compare(a, b):
    print 'start compare', current_millis()
    for i in range(len(a)):
        if i >= len(b) or a[i] != b[i]: 
            print 'end compare', current_millis()
            return False
        time.sleep(.05)

    print 'end compare (returning true)', current_millis()
    return True
...
class Test:
    ...
    def GET(self):
        params = web.input()
        valid = insecure_compare(self.hmac.sign(bytes(params['file'])), bytes(params['signature']).decode('hex'))
        if valid:
            return 'valid'
        else:
