...
class Sha1Hash(object):
    ...
    def __init__(self, digest=None, length=None):
        ...
        if digest is not None:
            self._h = self.undigest(digest)
            self._message_byte_length = length

    def set_registers(self, a, b, c, d, e):
        self._h = (a, b, c, d, e)

    def set_length(self, l):
        self._message_byte_length = l
    ...
    # Given a 160 bit binary string, return a collection of 5 32 bit integers
    def undigest(self, d):
        nums = []
        for i in range(5):
            chunk = d[i*4:i*4 + 4]
            nums.append(struct.unpack('>I', chunk)[0])
        return nums

    def digest(self):
        """Produce the final hash value (big-endian) as a bytes object"""
        d = self._produce_digest()
        chunks = [struct.pack(b'>I', h) for h in d]
    ...
    def _produce_digest(self):
        ...
        #print('bit length', message_bit_length)
        message += Sha1Hash.get_padding(message_byte_length)
