...
class MD4(object):
    def __init__(self, data="", digest=None, length=None):
        self.remainder = data
        self.count = 0
        self.h = [
                0x67452301,
        ...
        if digest is not None:
            self.h = self.undigest(digest)
            self.count = length / 64
    ...
    def update(self, data):
        ...
        for chunk in xrange(0, len(message)-r, 64):
            self._update_chunk( message[chunk:chunk+64] )
        return self

    def digest(self):
        l = len(self.remainder) + 64 * self.count
        self.update(MD4.get_padding(l))
    ...
    def get_padding(byte_length):
        return "\x80" + "\x00" * ((55 - byte_length) % 64) + struct.pack("<Q", byte_length * 8)

    def undigest(self, digest):
        chunks = []
        for i in range(4):
            chunks.append(struct.unpack('<I', digest[i*4:i*4+4])[0])
        return chunks
